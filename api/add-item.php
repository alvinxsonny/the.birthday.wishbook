...
// Get max sort order
$stmt = $db->prepare('SELECT COALESCE(MAX(sort_order), 0) + 1 as next_order FROM wishlist_items WHERE user_id = ?');
$stmt->execute([$_SESSION['user_id']]);
$nextOrder = $stmt->fetch()['next_order'];

// Insert
$stmt = $db->prepare('INSERT INTO wishlist_items (user_id, item_name, item_url, image_url, category_id, sort_order) VALUES (?, ?, ?, ?, ?, ?)');
$stmt->execute([
    $_SESSION['user_id'],
    $itemName,
    $itemUrl ?: null,
